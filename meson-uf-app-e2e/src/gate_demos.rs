...
/// Not a `table:id` shape — `RecordId::parse` rejects it, so any handler
/// calling `session_user_record_id` fails with a real `auth:` error.
#[cfg(feature = "ssr")]
pub const E2E_BROKEN_SESSION_USER: &str = "not-a-valid-record-id";
...
pub enum E2eAuthKind {
    /// Signed out.
    Anonymous,
    /// Owns the seeded fixture files.
    Owner,
    /// A second user — used for the foreign-id / IDOR sad path.
    Peer,
    /// Authenticated but email unverified.
    Unverified,
    /// Authenticated with a session user id that isn't a valid Valence
    /// `RecordId` — drives the My Files list/preview `io`/`auth` error path.
    BrokenSession,
}
...
impl E2eAuthKind {
    ...
    pub fn parse(raw: &str) -> Self {
        match raw {
            "owner" | "authenticated_verified" => Self::Owner,
            "peer" => Self::Peer,
            "unverified" | "authenticated_unverified" => Self::Unverified,
            "broken_session" => Self::BrokenSession,
            _ => Self::Anonymous,
        }
    }
    ...
    pub const fn session_user_id(self) -> Option<&'static str> {
        match self {
            ...
            Self::BrokenSession => Some(E2E_BROKEN_SESSION_USER),
        }
    }
    ...
    pub fn to_session(self) -> AuthSession {
        match self {
            ...
            Self::BrokenSession => AuthSession::Authenticated(AuthenticatedUser {
                user_id: "broken".into(),
                email: Some("broken@example.com".into()),
                display_name: Some("Broken Session".into()),
                avatar_url: None,
                roles: vec!["user".into()],
                email_verified: true,
            }),
    ...
    view! {
        <Suspense fallback=|| {
            ...
            {move || {
                let kind = auth_kind
                    .get()
                    .map_or(E2eAuthKind::Anonymous, |raw| E2eAuthKind::parse(&raw));
