//! Axum + Leptos host for meson-app `MesonRoutes` Playwright.

#![allow(clippy::print_stdout, missing_docs)]
// Boot-time setup: panicking here (logger, executor, leptos config) is the
// correct behavior for a lab host — there is no caller to recover into.
...
use meson_uf_app_e2e::seed::seed_data;
use meson_uf_app_e2e::{
...
async fn serve() -> anyhow::Result<()> {
    ...
    log::info!("meson-uf-app-e2e listening on http://{addr}");
